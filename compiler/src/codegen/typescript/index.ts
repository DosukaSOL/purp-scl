...
export class TypeScriptCodegen {
  ...
  private emitFrontendStandalone(): void {
    ...
    this.emit('// Auto-generated by Purp SCL Compiler v2.0.0 — Frontend Output');
  ...
  private emitHeader(): void {
    this.emit('// Auto-generated by Purp SCL Compiler v2.0.0');
    this.emit('// Do not edit manually');
    this.emit('');
  ...
  private emitInstructionMethod(node: AST.InstructionDeclaration): void {
    ...
    const allParams = [...accountParams, ...valueParams, 'signers: Keypair[]'];
    ...
    // Build instruction data: discriminator byte + borsh-serialized args
    if (node.params.length > 0) {
      this.emit('const data = Buffer.concat([');
      this.indent++;
      this.emit(`Buffer.from([${this.instructionIndex}]),`);
      for (const p of node.params) {
        const pName = this.toCamelCase(p.name);
        this.emit(`${this.emitBorshSerialize(pName, p.type)},`);
      }
      this.indent--;
      this.emit(']);');
    } else {
      this.emit(`const data = Buffer.from([${this.instructionIndex}]);`);
    }
    this.emit('');
    ...
    for (const acc of node.accounts) {
      ...
      this.emit(`{ pubkey: ${accName}, isSigner: ${isSigner}, isWritable: ${isMut} },`);
    }
    this.indent--;
    this.emit('],');
    this.emit('programId: this.programId,');
    this.emit('data,');
    this.indent--;
    this.emit('});');
    this.emit('');
    ...
    this.emit('const tx = new Transaction().add(ix);');
    this.emit('tx.recentBlockhash = (await this.connection.getLatestBlockhash()).blockhash;');
    this.emit('tx.feePayer = signers[0].publicKey;');
    this.emit('tx.sign(...signers);');
    this.emit('const sig = await this.connection.sendRawTransaction(tx.serialize());');
    this.emit('await this.connection.confirmTransaction(sig);');
    this.emit('return sig;');
  ...
  private emitBorshSerialize(varName: string, type: AST.TypeAnnotation): string {
    if (type.kind === 'PrimitiveType') {
      switch (type.name) {
        case 'u8': return `Buffer.from([${varName}])`;
        case 'u16': { return `(() => { const b = Buffer.alloc(2); b.writeUInt16LE(${varName}); return b; })()`; }
        case 'u32': { return `(() => { const b = Buffer.alloc(4); b.writeUInt32LE(${varName}); return b; })()`; }
        case 'u64': case 'u128':
        case 'i64': case 'i128':
          return `new BN(${varName}).toArrayLike(Buffer, 'le', ${type.name === 'u128' || type.name === 'i128' ? 16 : 8})`;
        case 'i8': return `Buffer.from([${varName} & 0xff])`;
        case 'i16': { return `(() => { const b = Buffer.alloc(2); b.writeInt16LE(${varName}); return b; })()`; }
        case 'i32': { return `(() => { const b = Buffer.alloc(4); b.writeInt32LE(${varName}); return b; })()`; }
        case 'f32': { return `(() => { const b = Buffer.alloc(4); b.writeFloatLE(${varName}); return b; })()`; }
        case 'f64': { return `(() => { const b = Buffer.alloc(8); b.writeDoubleLE(${varName}); return b; })()`; }
        case 'bool': return `Buffer.from([${varName} ? 1 : 0])`;
        case 'string': return `Buffer.concat([(() => { const b = Buffer.alloc(4); b.writeUInt32LE(Buffer.from(${varName}).length); return b; })(), Buffer.from(${varName})])`;
        case 'pubkey': return `${varName}.toBuffer()`;
  ...
  private emitAccountFetcher(acc: AST.AccountDeclaration): void {
    ...
    this.emit('try {');
    this.indent++;
    this.emit('const accountInfo = await this.connection.getAccountInfo(address);');
    this.emit('if (!accountInfo || accountInfo.data.length < 8) return null;');
    this.emit('const data = accountInfo.data.slice(8);');

    // Generate inline Borsh deserialization
    this.emit('let offset = 0;');
    for (const field of acc.fields) {
      const fName = this.toCamelCase(field.name);
      this.emit(this.emitBorshDeserialize(fName, field.type));
    ...
    this.indent++;
    this.emit('const accounts = await this.connection.getProgramAccounts(this.programId);');
    this.emit(`return accounts.filter(a => a.account.data.length >= 8).map(({ pubkey, account: acc }) => {`);
    this.indent++;
    this.emit('const data = acc.data.slice(8);');
    this.emit('let offset = 0;');
    for (const field of acc.fields) {
      const fName = this.toCamelCase(field.name);
      this.emit(this.emitBorshDeserialize(fName, field.type));
    }
    this.emit(`return { publicKey: pubkey, account: { ${fieldNames} } };`);
    this.indent--;
    this.emit('});');
    this.indent--;
    this.emit('}');
  ...
  private emitBorshDeserialize(varName: string, type: AST.TypeAnnotation): string {
    if (type.kind === 'PrimitiveType') {
      switch (type.name) {
        case 'u8': return `const ${varName} = data[offset]; offset += 1;`;
        case 'u16': return `const ${varName} = data.readUInt16LE(offset); offset += 2;`;
        case 'u32': return `const ${varName} = data.readUInt32LE(offset); offset += 4;`;
        case 'u64': return `const ${varName} = new BN(data.slice(offset, offset + 8), 'le'); offset += 8;`;
        case 'u128': return `const ${varName} = new BN(data.slice(offset, offset + 16), 'le'); offset += 16;`;
        case 'i8': return `const ${varName} = data.readInt8(offset); offset += 1;`;
        case 'i16': return `const ${varName} = data.readInt16LE(offset); offset += 2;`;
        case 'i32': return `const ${varName} = data.readInt32LE(offset); offset += 4;`;
        case 'i64': return `const ${varName} = new BN(data.slice(offset, offset + 8), 'le'); offset += 8;`;
        case 'f32': return `const ${varName} = data.readFloatLE(offset); offset += 4;`;
        case 'f64': return `const ${varName} = data.readDoubleLE(offset); offset += 8;`;
        case 'bool': return `const ${varName} = data[offset] !== 0; offset += 1;`;
        case 'string': return `const ${varName}Len = data.readUInt32LE(offset); offset += 4; const ${varName} = data.slice(offset, offset + ${varName}Len).toString('utf8'); offset += ${varName}Len;`;
        case 'pubkey': return `const ${varName} = new PublicKey(data.slice(offset, offset + 32)); offset += 32;`;
        default: return `const ${varName} = null; // unsupported type: ${type.name}`;
      }
    }
    return `const ${varName} = null; // complex type`;
  }

  private emitEventListener(): void {
    this.emit('addEventListener(');
    this.indent++;
    this.emit('callback: (logs: string[], signature: string) => void');
    this.indent--;
    this.emit('): number {');
    this.indent++;
    this.emit('return this.connection.onLogs(this.programId, (logInfo) => {');
    this.indent++;
    this.emit('callback(logInfo.logs, logInfo.signature);');
    this.indent--;
    this.emit('});');
    this.indent--;
    this.emit('}');
    ...
    this.indent++;
    this.emit('await this.connection.removeOnLogsListener(listenerId);');
    this.indent--;
    this.emit('}');
    this.emit('');
  }
  ...
  private emitPDAHelpers(): void {
    ...
    this.indent++;
    this.emit('return PublicKey.findProgramAddress(seeds, programId ?? this.programId);');
    this.indent--;
    this.emit('}');
    this.emit('');
